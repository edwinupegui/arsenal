...
The system MUST render sections in this fixed order: **Overdue → Due Today → Upcoming (next 7 days) → Recent Resources → This Month's Spending → Recent Transactions → Today's Events → Upcoming Events**. Empty sections SHALL be omitted from the rendered output (no header shown for zero-item sections). This order is not user-configurable in v3.0.
