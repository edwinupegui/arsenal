// Package sqliteutil centralizes the SQLite connection and migration concerns
// shared by every domain package (store, todos, finance, calendar). The goal
// is to keep DSN pragmas, driver registration, and goose configuration in
// exactly one place so that adding a new domain never duplicates that
// boilerplate.
package sqliteutil
...
// 5s busy timeout. The driver is the pure-Go modernc.org/sqlite — no CGO.
